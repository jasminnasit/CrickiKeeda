	...
	<div class="startContainer">
			<?php
				...
				if (($news==$teama && $inn=='i1') || ($news==$teamb && $inn=='i2') ) {
					$playersi1Bat=mysqli_query($dbase,"SELECT `pname`,`pid` FROM `players` WHERE `pid` IN (SELECT `pid` FROM `teamalloc` NATURAL JOIN `scores` WHERE `matchid`='$matchids' AND `tid`='$teamaid')");
					$playersi2Bowl=mysqli_query($dbase,"SELECT `pname`,`pid` FROM `players` WHERE `pid` IN (SELECT `pid` FROM `teamalloc` NATURAL JOIN `scores` WHERE `matchid`='$matchids' AND `tid`='$teamaid')");;
					$playersi1Bowl=mysqli_query($dbase,"SELECT `pname`,`pid` FROM `players` WHERE `pid` IN (SELECT `pid` FROM `teamalloc` NATURAL JOIN `scores` WHERE `matchid`='$matchids' AND `tid`='$teambid')");
					$playersi2Bat=mysqli_query($dbase,"SELECT `pname`,`pid` FROM `players` WHERE `pid` IN (SELECT `pid` FROM `teamalloc` NATURAL JOIN `scores` WHERE `matchid`='$matchids' AND `tid`='$teambid')");;
					...
					while ($player=mysqli_fetch_assoc($playersi1Bat)) {
						$pid=$player['pid'];
						$pname=$player['pname'];
						$details=mysqli_query($dbase,"SELECT `runs`,`inballs`,`takenby` FROM `scores` WHERE `pid`='$pid' AND `matchid`='$matchids'") ;
					...
					while ($player=mysqli_fetch_assoc($playersi1Bowl)) {
						$pid=$player['pid'];
						$pname=$player['pname'];
						$details=mysqli_query($dbase,"SELECT `over`,`wicket` FROM `scores` WHERE `pid`='$pid' AND `matchid`='$matchids'") ;
						$details=mysqli_fetch_assoc($details);
						$over=$details['over'];
				...
				elseif (($news==$teamb && $inn=='i1') || ($news==$teama && $inn=='i2') ) {
					$playersi1Bat=mysqli_query($dbase,"SELECT `pname`,`pid` FROM `players` WHERE `pid` IN (SELECT `pid` FROM `teamalloc` NATURAL JOIN `scores` WHERE `matchid`='$matchids' AND `tid`='$teambid')");
					$playersi2Bowl=mysqli_query($dbase,"SELECT `pname`,`pid` FROM `players` WHERE `pid` IN (SELECT `pid` FROM `teamalloc` NATURAL JOIN `scores` WHERE `matchid`='$matchids' AND `tid`='$teambid')");;
					$playersi1Bowl=mysqli_query($dbase,"SELECT `pname`,`pid` FROM `players` WHERE `pid` IN (SELECT `pid` FROM `teamalloc` NATURAL JOIN `scores` WHERE `matchid`='$matchids' AND `tid`='$teamaid')");
					$playersi2Bat=mysqli_query($dbase,"SELECT `pname`,`pid` FROM `players` WHERE `pid` IN (SELECT `pid` FROM `teamalloc` NATURAL JOIN `scores` WHERE `matchid`='$matchids' AND `tid`='$teamaid')");;

					echo "<table class='pastScoreTable'><tr><td colspan='5'>Inning-1 Batting Team</td></tr><tr><th>Id</th><th>Name</th><th>Runs</th><th>Balls</th><th>Wicket By</th></tr>";
					while ($player=mysqli_fetch_assoc($playersi1Bat)) {
						$pid=$player['pid'];
						$pname=$player['pname'];
						$details=mysqli_query($dbase,"SELECT `runs`,`inballs`,`takenby` FROM `scores` WHERE `pid`='$pid' AND `matchid`='$matchids'") ;
					...
					echo "<table class='pastScoreTable'><tr><td colspan='4'>Inning-1 Bowling Team</td></tr><tr><th>Id</th><th>Name</th><th>Overs</th><th>Wickets</th></tr>";
					while ($player=mysqli_fetch_assoc($playersi1Bowl)) {
						$pid=$player['pid'];
						$pname=$player['pname'];
						$details=mysqli_query($dbase,"SELECT `over`,`wicket` FROM `scores` WHERE `pid`='$pid' AND `matchid`='$matchids'") ;
					...
					echo "<table class='pastScoreTable'><tr><td colspan='5'>Inning-2 Batting Team</td></tr><tr><th>Id</th><th>Name</th><th>Runs</th><th>Balls</th><th>Wicket By</th></tr>";
					while ($player=mysqli_fetch_assoc($playersi2Bat)) {
						$pid=$player['pid'];
						$pname=$player['pname'];
						$details=mysqli_query($dbase,"SELECT `runs`,`inballs`,`takenby` FROM `scores` WHERE `pid`='$pid' AND `matchid`='$matchids'") ;
					...
					echo "<table class='pastScoreTable'><tr><td colspan='4'>Inning-2 Bowling Team</td></tr><tr><th>Id</th><th>Name</th><th>Overs</th><th>Wickets</th></tr>";
					while ($player=mysqli_fetch_assoc($playersi2Bowl)) {
						$pid=$player['pid'];
						$pname=$player['pname'];
						$details=mysqli_query($dbase,"SELECT `over`,`wicket` FROM `scores` WHERE `pid`='$pid' AND `matchid`='$matchids'") ;
